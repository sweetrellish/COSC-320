...
public:
  ...
  // constructor
  stnode(const T &item, stnode<T> *lptr = NULL, stnode<T> *rptr = NULL,
         stnode<T> *pptr = NULL)
...
private:
  ...
  stnode<T> *getSTNode(const T &item, stnode<T> *lptr, stnode<T> *rptr,
                       stnode<T> *pptr);
...
template <typename T> bool stree<T>::graphWinOpen = false;

template <typename T>
stnode<T> *stree<T>::getSTNode(const T &item, stnode<T> *lptr, stnode<T> *rptr,
                               stnode<T> *pptr) {
  stnode<T> *newNode;
...
template <typename T> void stree<T>::deleteTree(stnode<T> *t) {
...
template <typename T> stree<T>::stree() : root(NULL), treeSize(0) {}
...
template <typename T> stree<T>::~stree() {
...
template <typename T> stree<T> &stree<T>::operator=(const stree<T> &rhs) {
...
template <typename T> int stree<T>::empty() const { return root == NULL; }

template <typename T> int stree<T>::size() const { return treeSize; }

template <typename T> void stree<T>::erase(iterator pos) {
...
template <typename T> int stree<T>::erase(const T &item) {
  int numberErased = 1;
  // search tree for item
...
template <typename T> void stree<T>::erase(iterator first, iterator last) {
  if (treeSize == 0)
    throw underflowError("stree erase(): tree is empty");
...
template <typename T> void stree<T>::displayTree(int maxCharacters) {
  string label;
...
template <typename T> void stree<T>::deleteShadowTree(stnodeShadow *t) {
  // if current root node is not NULL, delete its left subtree,
  // its right subtree and then the node itself
  if (t != NULL) {
#ifndef BINARY_SEARCH_TREE_CLASS
...
#include "d_stiter.h"
    ...
    template <typename T>
    stnode<T> *stree<T>::getSTNode(const T &item, stnode<T> *lptr,
    ...
    // recursive inorder scan used to build the shadow tree
    template <typename T>
    stnodeShadow *stree<T>::buildShadowTree(stnode<T> * t, int level,
                                            int &column) {
      // pointer to new shadow tree node
      stnodeShadow *newNode = NULL;
      // text and ostr used to perform format conversion
    ...
    template <typename T> void stree<T>::drawTree(int maxCharacters) {
      drawTreeFrame(maxCharacters, true);
    }

    template <typename T> void stree<T>::drawTrees(int maxCharacters) {
    ...
    cellSide/2.0 + 0.1; childy = currNode->left->level * cellSide + cellSide/2.0
    + 0.1; edge.setEndPoint(childx, childy); edge.draw();
                            q.push(currNode->left);
                    }
    ...
    cellSide/2.0 + 0.1; childy = currNode->right->level * cellSide +
    cellSide/2.0 + 0.1; edge.setEndPoint(childx, childy); edge.draw();
                            q.push(currNode->right);
                    }
                    ...
                    node.move(x,y);
                    node.draw();
                    // draw the node data. use an appropriate offset from the
    node
...
void stree<T>::drawTreeFrame(int maxCharacters, bool simpleDraw)
{
   ...
   while(!q.empty())
   {
                ...
                if(currNode->left != NULL)
                {
                        ...
                        childx = currNode->left->column * cellSide +
cellSide/2.0 + 0.1; childy = currNode->left->level * cellSide + cellSide/2.0 +
0.1; edge.setEndPoint(childx, childy); edge.draw(); q.push(currNode->left);
                }
      ...
      if(currNode->right != NULL)
                {
                        ...
                        // compute the center of the right child node
                        childx = currNode->right->column * cellSide +
cellSide/2.0 + 0.1; childy = currNode->right->level * cellSide + cellSide/2.0 +
0.1; edge.setEndPoint(childx, childy); edge.draw(); q.push(currNode->right);
                }
